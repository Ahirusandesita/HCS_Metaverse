...
public class PlacingTarget : MonoBehaviour, IDependencyInjector<PlayerBodyDependencyInformation>, IInputControllable
{
	...
	protected const float ROTATE_DURATION = 60f;  // 1秒間に回転する角度（度数法）
	...
	protected virtual bool PreviewPlacing()
	{
		...
		Ray checkGroundRay_rf = new Ray(underOrigin + boxHalfSize.x * transform.right + boxHalfSize.z * transform.forward + Vector3.up * stepHeightLimit, Vector3.down);
		Ray checkGroundRay_lf = new Ray(underOrigin + -boxHalfSize.x * transform.right + boxHalfSize.z * transform.forward + Vector3.up * stepHeightLimit, Vector3.down);
		Ray checkGroundRay_rb = new Ray(underOrigin + boxHalfSize.x * transform.right + -boxHalfSize.z * transform.forward + Vector3.up * stepHeightLimit, Vector3.down);
		Ray checkGroundRay_lb = new Ray(underOrigin + -boxHalfSize.x * transform.right + -boxHalfSize.z * transform.forward + Vector3.up * stepHeightLimit, Vector3.down);
