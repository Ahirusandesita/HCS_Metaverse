...
public class FoodSpawnManager : MonoBehaviour, ISelectedNotification
{
    ...
    {
        Debug.LogError("存在しているか" + GateOfFusion.Instance.NetworkRunner.TryFindBehaviour(networkBehaviourId, out NetworkBehaviour networkBehaviour));
    }
    ...
    public void Despawn(NetworkView networkView)
    {
        ...
        foreach (NetworkInformation item in networkInformations)
        {
            ...
            {
                Debug.LogError("NetworkView一致");
                information = item;
            }
        }
        if(information == null)
        {
            Debug.LogError("NetworkViewの不一致");
            return;
        }
