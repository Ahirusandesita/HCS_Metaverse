...
public class ThrowDataReWark : MonoBehaviour
{
    #region コンストラクタ
    /// <summary>
    ...
    public ThrowDataReWark(Vector3 nowPosition)
    {
        // 軌道座標の初期化を行う
    ...
    public Vector3 GetThrowVector()
    {
        ...
        // 経過時間から時間の平均係数を求める
        float averageTime = 1 / _orbitDatas[0]._storeTime - _orbitDatas[usableIndex]._storeTime;

        // 軌道ベクトルと時間の平均係数から投擲速度を生成する
        throwVelocity /= averageTime;
